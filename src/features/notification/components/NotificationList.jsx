...
import { getNotifications, getNotificationsByType, markAsRead, markAllAsRead } from '../services/NotificationService';
...
const NotificationList = ({ activeFilter }) => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (activeFilter === 'all') {
        ...
        // Map UI filters to backend types
        let typesToFetch = [];
        switch (activeFilter) {
          case 'posts': 
            typesToFetch = ['SHARE_POST']; 
            break;
          case 'comments': 
            typesToFetch = ['COMMENT_POST', 'REPLY_COMMENT']; 
            break;
          case 'upvotes': 
            typesToFetch = ['LIKE_POST', 'LIKE_COMMENT']; 
            break;
          case 'profile': 
            typesToFetch = ['NEW_FOLLOWER']; 
            break;
          case 'systems': 
            typesToFetch = ['SYSTEM_ALERT', 'REPORT_RESOLVED']; 
            break;
          default:
            typesToFetch = [];
        }
  ...
  const handleNotificationClick = async (notif) => {
    if (!notif.isRead) {
      try {
        ...
        setNotifications(prev => prev.map(n => 
          (n.id === notif.id || n._id === notif._id) ? { ...n, isRead: true } : n
        ));
      } catch (error) {
    ...
    // This depends on how the backend sends the target IDs.
    if (notif.postId) {
      navigate(`/post/${notif.postId}`);
    } else if (notif.type === 'NEW_FOLLOWER' && notif.sender) {
      navigate(`/profile/${notif.sender.id || notif.sender._id}`);
    }
  };
  ...
  if (loading) {
    return (
      <main className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden w-full flex-1 p-6 flex justify-center items-center">
        <div className="w-6 h-6 border-2 border-[#8d3f41] border-t-transparent rounded-full animate-spin"></div>
      </main>
    );
  }
    ...
    <main className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden w-full flex-1">
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-white sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2 text-[13px] text-gray-500">
          <button onClick={handleMarkAllRead} className="hover:underline transition-colors hover:text-[#8d3f41]">
            ...
            return (
              ...
              >
                ...
                <div className="flex-shrink-0 w-8 h-8 mt-1 rounded-md overflow-hidden bg-gray-100 flex items-center justify-center">
                  <img src={notif.actor?.avatarUrl || getIconUrl(notif)} alt="icon" className="w-full h-full object-cover" />
                </div>
                ...
                <div className="absolute right-4 top-4 text-gray-400 group-hover:text-gray-700 transition-colors opacity-0 group-hover:opacity-100">
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      if (isUnread) handleNotificationClick(notif);
                    }}
